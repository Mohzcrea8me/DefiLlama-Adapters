...
function isLP(symbol, token, chain) {
  ...
  if (
    (token && blacklisted_LPS.includes(token.toLowerCase())) ||
    symbol.includes("HOP-LP-")
  )
    return false;
  if (
    chain === "bsc" &&
    ["OLP", "DLP", "MLP", "LP", "Stable-LP", "fCake-LP", "fMDEX LP"].includes(
      symbol
    )
  )
    return false;
  if (chain === "bsc" && ["WLP", "FstLP", "BLP", "DsgLP"].includes(symbol))
    return true;
  if (chain === "pulse" && ["PLP", "PLT"].includes(symbol)) return true;
  if (
    chain === "avax" &&
    ["ELP", "EPT", "CRL", "YSL", "BGL", "PLP"].includes(symbol)
  )
    return true;
  ...
  if (["fantom", "nova"].includes(chain) && ["NLT"].includes(symbol))
    return true;
  let label;

  if (
    symbol.startsWith("ZLK-LP") ||
    symbol.includes("DMM-LP") ||
    (chain === "avax" && "DLP" === symbol) ||
    symbol === "fChe-LP"
  )
  ...
  const isLPRes =
    LP_SYMBOLS.includes(symbol) ||
    /(UNI-V2|vAMM|sAMM)/.test(symbol) ||
    symbol.split(/\W+/).includes("LP");
...
function mergeExports(...exportsArray) {
  ...
  exportsArray.forEach((exportObj) => {
    Object.keys(exportObj).forEach((key) => {
      ...
      Object.keys(exportObj[key]).forEach((key1) =>
        addToExports(key, key1, exportObj[key][key1])
      );
    });
  });
  ...
  Object.keys(exports)
    ...
    .forEach((chain) => {
      ...
      Object.keys(obj).forEach((key) => {
        if (obj[key].length > 1) obj[key] = sdk.util.sumChainTvls(obj[key]);
        else obj[key] = obj[key][0];
      });
    });
...
async function getBalance(chain, account) {
  switch (chain) {
    case "bitcoin":
      return (
        (await http.get(`https://chain.api.btc.com/v3/address/${account}`)).data
          .balance / 1e8
      );
    case "bep2":
      const balObject = (
        await http.get(
          `https://api-binance-mainnet.cosmostation.io/v1/account/${account}`
        )
      ).balances.find((i) => i.symbol === "BNB");
...
async function diplayUnknownTable({
  tvlResults = {},
  tvlBalances = {},
  storedKey = "ethereum",
  tableLabel = "Unrecognized tokens",
}) {
...
async function debugBalances({
  ...
  Object.keys(balances).forEach((label) => {
    ...
    if (!label.startsWith(chain)) ethTokens.push(token);
  ...
  if (withETH) {
    ...
    symbolsETH = await ethApi.multiCall({
      abi: "erc20:symbol",
      calls: ethTokens,
      permitFailure: true,
    });
    nameETH = await ethApi.multiCall({
      abi: erc20.name,
      calls: ethTokens,
      permitFailure: true,
    });
  ...
  let symbolMapping = symbols.reduce(
    (a, i, y) => ({ ...a, [tokens[y]]: i }),
    {}
  );
  let decimalsMapping = decimals.reduce(
    (a, i, y) => ({ ...a, [tokens[y]]: i }),
    {}
  );
  let nameMapping = name.reduce((a, i, y) => ({ ...a, [tokens[y]]: i }), {});
  if (withETH) {
    symbolMapping = symbolsETH.reduce(
      (a, i, y) => ({ ...a, [ethTokens[y]]: i }),
      symbolMapping
    );
    nameMapping = nameETH.reduce(
      (a, i, y) => ({ ...a, [ethTokens[y]]: i }),
      nameMapping
    );
  }
  ...
  Object.entries(balances).forEach(([label, balance]) => {
    ...
    if (decimal) balance = (balance / 10 ** decimal).toFixed(0);
