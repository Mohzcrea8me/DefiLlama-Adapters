...
async function getLogs({
  ...
  if (onlyUseExistingCache) toBlock = 1e11;
  else await api.getBlock();
  ...
  if (eventAbi) {
    ...
    if (!topics?.length) {
      ...
      topics = undefined;
      topic = `${fragment.name}(${fragment.inputs
        .map((i) => i.type)
        .join(",")})`;
    }
  }
  ...
  if ((cache.fromBlock && cache.toBlock > toBlock) || onlyUseExistingCache)
    response = cache.logs.filter(
      (i) => i.blockNumber < toBlock && i.blockNumber >= fromBlock
    );
  else response = await fetchLogs();
  ...
  async function fetchLogs() {
    ...
    if (!skipCache) await setCache(cacheFolder, key, cache);
